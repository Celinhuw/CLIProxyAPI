...
class TestN8nTrigger(unittest.IsolatedAsyncioTestCase):
    ...
    @patch("tools.n8n_bridge.requests.post")
    @patch("tools.n8n_bridge._get_n8n_config", return_value=("http://localhost:5678", "key"))
    async def test_successful_trigger(self, mock_config, mock_post):
...
class TestN8nStatus(unittest.IsolatedAsyncioTestCase):
    ...
    @patch("tools.n8n_bridge._n8n_request")
    @patch("tools.n8n_bridge.requests.get")
    async def test_healthy(self, mock_get, mock_req):
        mock_get.return_value = MagicMock(status_code=200)
        mock_req.return_value = {"data": []}
        result = await n8n_status()
        self.assertIn("Running", result)

    @patch("tools.n8n_bridge.requests.get", side_effect=__import__("requests").ConnectionError("refused"))
    async def test_not_running(self, _):
        result = await n8n_status()
        self.assertIn("not running", result)
